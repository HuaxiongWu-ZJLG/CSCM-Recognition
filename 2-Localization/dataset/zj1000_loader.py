...
ctw_root_dir = './data/zj/'
ctw_train_data_dir = ctw_root_dir + 'train/text1/'
ctw_train_gt_dir = ctw_root_dir + 'train/text_label1/'
# ctw_test_data_dir = ctw_root_dir + 'test/original_text_image/'   ##我自己更改了文件名
# ctw_test_gt_dir = ctw_root_dir + 'test/text_label_curve/'
##使用自己的数据集（8点标注的）,,还有下面bbox坐标点改动，，，img.shape[0]] * 8改动,改动成功
# ctw_root_dir = './data/zj/'
# ctw_train_data_dir = ctw_root_dir + 'train/text/'
# ctw_train_gt_dir = ctw_root_dir + 'train/text_label/'
# ctw_test_data_dir = ctw_root_dir + 'test/original_text_image/'   ##我自己更改了文件名
# ctw_test_gt_dir = ctw_root_dir + 'test/text_label_curve/'
...
def get_img(img_path):
    try:
        ...
        img = img[:, :, [2, 1, 0]]    #等同img=img[；，：，：：-1]，由于opencv里imwrite函数读取图像的颜色通道为BGR，转换颜色通道为RGB（正常显示的图片）
...
def get_bboxes(img, gt_path):
    ...
    for line in lines:
        ...
        # bbox = [np.int(gt[i]) for i in range(4, 32)]  ###txt里面两个矩形角点坐标，14个上下点坐标，合计4+28=32
        # bbox = np.asarray(bbox) + ([x1 * 1.0, y1 * 1.0] * 14)
        # bbox = np.asarray(bbox) / ([w * 1.0, h * 1.0] * 14)
        ##改动坐标点8个
        try:
            bbox = [int(gt[i]) for i in range(16)]  ###txt里面顺时针8个上下点坐标，合计16
            # print(bbox)
            bbox = np.asarray(bbox) / ([w * 1.0, h * 1.0] * 8)
            bboxes.append(bbox)
            tags.append(True)
        except:
            print('load gt failed on {}',format(gt_path))
...
class CTW1500Loader(data.Dataset):
    def __init__(self, is_transform=False, img_size=None, kernel_num=4, min_scale=0.7):
    ...
    def __getitem__(self, index):
        ...
        if bboxes.shape[0] > 0:
            bboxes = np.reshape(bboxes * ([img.shape[1], img.shape[0]] * 8), (bboxes.shape[0], bboxes.shape[1] // 2, 2)).astype('int32')   #float报错，由于版本问题//才是整除，非/（python2）
