...
def adjust_learning_rate(args,optimizer,epoch):
    ...
    if epoch < 6:
        # print(args.lr)
        lr = 1e-5 + (args.lr - 1e-5) * epoch / 6
        # print(epoch)
    else:

        lr = args.lr * (0.1 ** (epoch / args.schedule[0]))
...
def main(args):
    if args.checkpoint == '':
        args.checkpoint = "checkpoints/pre_zj1080_%s_bs_%d_ep_%d"%(args.arch, args.batch_size, args.n_epoch)
    if args.pretrain:
    ...
    #     model = models.resPAnet152(pretrained=True, num_classes=kernel_num)
        ...
        model = models.resCAnet50(pretrained=True, num_classes=kernel_num)
    elif args.arch == "resCAnet101":
        model = models.resCAnet101(pretrained=True, num_classes=kernel_num)
    elif args.arch == "resCAnet152":
        model = models.resCAnet152(pretrained=True, num_classes=kernel_num)
    ...
    logger.close()
##采用SGD优化器，动量0.99，weight decay=5*e-4
##原文在9000数据集IC17上训练180K，是4卡，微调：16——batchsize，epoch=36K，，在12k前，lr=1e-3，12k后，lr=1e-4
if __name__ == '__main__':
    ...
    #checkpoints/zj1000_resCAnet50_bs_4_ep_600/checkpoint.pth.tar
    ##预训练模型
    # parser.add_argument('--pretrain', nargs='?', type=str, default='model/CCA_ic15_MLT_CTW_600.pth.tar',
    #                     help='Path to previous saved model to restart from')
